#!/bin/bash
echo "Use this to build a production image of the cataas-client-uwsgi"
...
then
    ...
    cd ..
	git archive --format=tar.gz $VERSION django_project > deployment/docker/cataas-client.tar.gz
    cd -
    ...
    echo "ADD cataas-client.tar.gz /home/web" >> docker/Dockerfile-prod
    echo "CMD [\"uwsgi\", \"--ini\", \"/uwsgi.conf\"]" >> docker/Dockerfile-prod
    # Now build the image and tag it
    cd docker
	docker build -t 3yourmind/cataas-uwsgi:latest -f Dockerfile-prod .
    docker tag 3yourmind/cataas-uwsgi:latest 3yourmind/cataas-client-uwsgi:$VERSION
    docker push 3yourmind/cataas-uwsgi:latest
    docker push 3yourmind/cataas-uwsgi:$VERSION
    cd -
	# Clean up
